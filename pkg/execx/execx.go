...
func Run(arg, dir string, in ...*bytes.Buffer) (string, error) {
	...
	var cmd *exec.Cmd
	switch goos {
	case consts.OsMac, consts.OsLinux:
	...
	if err != nil {
		if stderr.Len() > 0 {
			return "", errors.New(strings.TrimSuffix(stderr.String(), consts.NL))
		}
		return "", err
	}

	return strings.TrimSuffix(stdout.String(), consts.NL), nil
}
